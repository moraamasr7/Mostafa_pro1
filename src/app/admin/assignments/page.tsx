...
export default function AdminAssignmentsPage() {
  ...
  // 1. جلب الطلبات الجاهزة للدليفري والطيارين المتاحين
  const fetchAssignmentData = async () => {
    ...
    try {
      ...
      if (ordersRes.ok && driversRes.ok) {
        ...
        // الطلبات الجاهزة للتعيين فقط (status = ready)
        const ready = allOrders.filter((o) => o.status === 'ready')

        // الطلبات المعينة والمتحركة نشطاً (assigned, picked_up, out_for_delivery)
        ...
        // الطيارون المتاحون للتعيين (active + open shift + available status)
  ...
  useEffect(() => {
    ...
    // Realtime channel لمزامنة شاشة التعيين فورياً عند أي تغيير
  ...
  // تعيين الطلب للطيار المختار عبر RPC
  ...
  // تحديث مرحلة التوصيل (picked_up ➔ out_for_delivery ➔ delivered)
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-gray-900 flex flex-col font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 flex-1 w-full space-y-6">
        ...
        {loading ? (
          ...
          <div className="space-y-8">
            {/* الشاشة الرئيسية: الجريان المباشر بين الطلبات الجاهزة والطيارين المتاحين */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              {/* قسم طلبات الدليفري الجاهزة (Ready Delivery Orders) */}
              ...
              {/* قسم الطيارين المتاحين (Available Drivers) */}
              <div className="lg:col-span-5 space-y-4">
                        ...
                        >
                          <div className="flex justify-between items-center">
                            <div>
                              ...
                              <span className="text-xs text-gray-500 font-semibold dir-ltr block">
                                📞 {driver.phone}
                              </span>
                            </div>
                            <span
                ...
                <div className="bg-white p-5 rounded-3xl border border-gray-200 shadow-lg space-y-3">
                  <div className="text-xs space-y-1">
                    ...
                    <p className="font-bold text-gray-900">
                      الطيار: {selectedDriver ? `${selectedDriver.name} (${selectedDriver.phone})` : 'لم يحدد'}
                    </p>
                  </div>
            ...
            {/* قسم الطلبات المعينة والمتحركة نشطاً (Active Delivery Progress) */}
                      ...
                      >
                        ...
                        {order.assigned_driver && (
                          <div className="bg-indigo-50 border border-indigo-200 p-2.5 rounded-xl text-xs space-y-1">
                            ...
                            <span className="text-[11px] text-indigo-700 font-semibold dir-ltr block">
      ...
      {/* نافذة إعادة تعيين الطلب لطيار آخر (Reassignment Modal) */}
      {reassigningOrder && (
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-gray-100">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {availableDrivers.map((d) => (
                ...
                >
                  <span className="font-extrabold text-gray-900">{d.name} ({d.phone})</span>
