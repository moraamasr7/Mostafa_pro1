...
export default function AdminDriversPage() {
  ...
  // 1. التحقق من صلاحية الجلسة وجلب البيانات
  const fetchDriversData = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // جلب تفاصيل وقت الوردية والطلب النشط إن وجد لكل طيار
  ...
  useEffect(() => {
    ...
    // Realtime channel لتحديث حالة الطيارين والورديات والتعيينات فورياً
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {drivers.map((d) => {
              ...
              return (
                ...
                >
                  {/* الرأس: الاسم والهاتف والحالة */}
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="text-base font-black text-gray-900">{d.name}</h3>
                      <a
                        href={`tel:${d.phone}`}
                        className="text-xs font-bold text-amber-700 hover:underline dir-ltr block mt-0.5"
                      >
                        📞 {d.phone}
                      </a>
                    </div>
