...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'pickup') {
      ...
      // تحديث حالة خط السير والطلبات المرتبطة بها لـ picked_up
      ...
      // تحديث تعينات وقواعد الطلبات الممثلة
    ...
    if (action === 'record_outcome') {
      ...
      // التأكد من أن الطلب يخص الطيار الحالي
      ...
      const { data: rpcData, error: rpcErr } = await supabase.rpc('record_delivery_outcome_secure', {
        p_order_id: order_id,
        p_outcome: outcome,
    ...
    if (action === 'complete_trip') {
      ...
      const { data: rpcData, error: rpcErr } = await supabase.rpc('complete_delivery_trip_secure', {
        p_trip_id: trip_id,
      })
